    ...
    <section>
        <aside>
            ...
            <ul>
                ...
                <li class="${cate eq 'fnq' ? 'on' : '' }"><a href="/farmstory/board/list?group=community&cate=fnq">자주묻는질문</a></li>
            </ul>
        </aside>
        <article>
            <nav>
                <img src="/farmstory/img/sub_nav_tit_cate5_${cate}.png" alt="${cate}"/>
                <p>
               		HOME > 커뮤니티 >
                	<c:if test="${cate eq 'notice'}"><span>공지사항</span></c:if>
                	<c:if test="${cate eq 'menu'}"><span>오늘의식단</span></c:if>
                	<c:if test="${cate eq 'chef'}"><span>나도요리사</span></c:if>
                	<c:if test="${cate eq 'qna'}"><span>고객문의</span></c:if>
                	<c:if test="${cate eq 'fnq'}"><span>자주묻는질문</span></c:if>
                </p>
            </nav>
            <div class="content">
                <!-- 컨텐츠 내용 시작  -->
